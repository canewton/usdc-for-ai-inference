...
import { isYesterday, isThisWeek, subDays } from "date-fns"
...
export function ChatSidebar({
  ...
  const renderChatItem = (chat: Chat) => (
    ...
    >
      ...
      >
        {chat.id === currentChatId && <TrashIcon className="h-4 w-4 ml-auto" color='#4D4861'/>}
      </button>
    </div>
  )
  ...
  const renderChatGroup = (title: string, chats: Chat[]) => {
    ...
    return (
      <div className='mb-6'>
        <h3 className="text-sm text-sub mb-2 uppercase tracking-wider">{title}</h3>
  ...
  return (
    <div className="h-[calc(100vh-317px)] pt-4 pb-8 px-2 rounded-r-3xl border border-gray-200 bg-section">
      <div className="overflow-auto h-full px-2">
        <button onClick={onNewChat} className="p-2 mb-4 text-left w-full h-fit hover:bg-gray-50 transition rounded-lg text-headline">
          New Chat
        </button>
