...
try {
  ...
  const createdWalletResponse = await circleDeveloperSdk.createWallets({
    accountType: 'SCA',
    blockchains: [process.env.CIRCLE_BLOCKCHAIN],
    walletSetId,
  });
  ...
  console.log(
    `Treasury wallet created successfully. Address: ${createdWallet.address}, ID: ${createdWallet.id}`,
  );
  ...
  if (profileError) {
    console.error('Error while attempting to save user:', profileError);
  }
  ...
  if (walletError) {
    console.error(
      'Error while attempting to store treasury wallet:',
      walletError,
    );
  }
} catch (error) {
