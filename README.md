...
9. **Run the generate-wallet.mjs script to craete the agent wallet and updated the environment variables**:

   ```bash
   npm run generate-treasury-wallet
   ```

10. **Create a new project on [Supabase](https://app.supabase.com)**
...
11. **Link your local project to your Supabase cloud project and push the database schema**:
   ```bash
...
12. **Deploy the wallet-transactions edge function**
   ```bash
   npx supabase functions deploy wallet-transactions --project-ref lsjrcexcbnebjpwquzei
   ```
