...
export const signUpAction = async (formData: FormData) => {
  ...
  if (signUpError) {
   ...
   if (!authData.user) {
     ...
     return encodedRedirect("error", "/sign-up", "Could not create user. Please try again.");
   }
  ...
  try {
    // 1. Create Wallet Set
    const createWalletSetResponse = await fetch(`${getBaseUrl()}/api/wallet-set`, {
      method: "PUT",
    ...
    if (!createWalletSetResponse.ok) {
      ...
      console.error(`Failed to create wallet set: ${createWalletSetResponse.status} ${errorBody}`);
    ...
    if (!walletSetId) {
        ...
        // Decide how to handle this - maybe delete the user?
         return encodedRedirect("error", "/sign-up", "Wallet setup failed (Set ID). Please contact support.");
    }
    ...
    const createWalletResponse = await fetch(`${getBaseUrl()}/api/wallet`, {
     ...
     if (!createWalletResponse.ok) {
       ...
       console.error(`Failed to create wallet: ${createWalletResponse.status} ${errorBody}`);
       // Critical failure - maybe delete user and wallet set?
       return encodedRedirect("error", "/sign-up", "Wallet setup failed (Wallet Creation). Please contact support.");
     }
    ...
    const walletAddress = createdWallet?.address;
     ...
     if (!circleWalletId || !walletAddress) {
       ...
       return encodedRedirect("error", "/sign-up", "Wallet setup failed (Wallet Details). Please contact support.");
     }
    ...
    if (profileError || !profileData) {
      console.error("Error updating/finding profile:", profileError?.message ?? "Profile not found");
       // Critical failure - user exists but profile doesn't match? Cleanup needed.
       return encodedRedirect("error", "/sign-up", "User profile setup failed. Please contact support.");
    }
    ...
    if (walletInsertError) {
      ...
      // Less critical maybe, but indicates inconsistency.
       return encodedRedirect("error", "/sign-up", "Wallet data saving failed. Please contact support.");
    }

  } catch (error: any) {
    console.error("Wallet creation process error:", error.message);
    // Generic catch-all
     return encodedRedirect("error", "/sign-up", "An unexpected error occurred during wallet setup.");
  }
  // ---- End Wallet Creation Logic ----

  // Redirect based on email confirmation setting
  const isEmailConfirmationEnabled = process.env.AUTH_EMAIL_CONFIRMATION_ENABLED === 'true'; // Example env var
