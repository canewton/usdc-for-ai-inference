...
import { GET } from '@/app/api/video-generation/route';
...
describe('GET /api/video-generation', () => {
  ...
  beforeEach(() => {
    ...
    mockRequest = {
      json: jest.fn(),
      headers: new Headers(),
      url: 'http://localhost/api/video-generation',
  ...
  it('should return 200 with generated videos for an authenticated user', async () => {
    ...
    expect(data).toEqual(mockGeneratedVideos);
  ...
  it('should return 500 if there is an error fetching generated videos', async () => {
    ...
    expect(data.error).toBe('Error fetching videos');
    expect(mockSupabase.from).toHaveBeenCalledWith('video_generations');
  });
});
